...
ssh $USER@$HOST "cd ""${DIR}"" && export PGPASSWORD=adelantado && pg_dump -h localhost -p 25432 -U adelantado -d adelantado -a --rows-per-insert=1000 --on-conflict-do-nothing -f dump-""${DATE}"".sql"
